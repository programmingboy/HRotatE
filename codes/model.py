...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, 
        ...
        self.relation_inv_embedding = nn.Parameter(torch.zeros(nrelation, self.relation_dim))
        nn.init.uniform_(
            tensor=self.relation_inv_embedding, 
            a=-self.embedding_range.item(), 
            b=self.embedding_range.item()
        )
    ...
    def forward(self, sample, mode='single'):
        ...
        if mode == 'single':
            ...
            tail = torch.index_select(
                self.entity2_embedding, 
                dim=0, 
                index=sample[:,2]
            ).unsqueeze(1)
            
            
            relation_inv = torch.index_select(
                self.relation_inv_embedding, 
                dim=0, 
                index=sample[:,1]
            ).unsqueeze(1)
            
            
            ht = torch.index_select(
                self.entity_embedding, 
                dim=0, 
                index=sample[:,2]
            ).unsqueeze(1)
    ...
    #def RotatE(self, head, relation, tail, mode):
        ...
        score1 = torch.stack([re_score, im_score], dim = 0)
        score1 = score1.norm(dim = 0)

        score1 = self.gamma.item() - score1.sum(dim = 2)
        ...
        phase_relation = relation_inv/(self.embedding_range.item()/pi)
        ...
        score2 = torch.stack([re_score, im_score], dim = 0)
        score2 = score2.norm(dim = 0)

        score2 = self.gamma.item() - score2.sum(dim = 2)
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        if args.regularization != 0.0:
            ...
            regularization = args.regularization * (
                ...
                model.entity2_embedding.norm(p = 3)**3 +
                model.relation_embedding.norm(p = 3).norm(p = 3)**3 +
                model.relation_inv_embedding.norm(p = 3).norm(p = 3)**3)/2 
